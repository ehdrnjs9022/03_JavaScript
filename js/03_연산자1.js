...
/* Number(input1.value)가 계속 반복되는데 전역변수로 선언하면 안될까?
    -> 안된다! 왜? 전역 변수는 페이지 로딩 시점에 해석되서 값이 결정된다
    -> input 에 작성된 값이 없어서 Number(input1.vale) == 0
    ->(해결방법) 
    값을 입력 한 후 함수를 호출해서 함수가 호출되는 시점의
    입력된 값을 읽어올수 있게 만든다!
*/
...
function plusFn(){
    ...
    // 단, '숫자형태 문자열'이 아닐경우 NaN(Not a Number) 결과 반환
...
// ex) +,-,*,/,%, ++, --,
//    변수에 값 대입, consol 출력, innerText...모두 다
...
function checkFn(){
    ...
    num = 100; 
    console.log("num++ :", num++); // 100 출력 후 -> num 1 증가(101)

    console.log("num++ :", num++); // 101 출력 후 -> num 1 증가(102)

    console.log("num++ :", num++); // 102 출력 후 -> num 1 증가(103) 
