...
function tagNameTest(){
    ...
    for(let i = 0 ; i < lists. length ; i++){
        // 각 요소에 저장된 값을 얻어와 색상 rgb 변경에 이용
...
function nameTest(){
    ...
    for(let i = 0; i < hobbyList.length; i++){
       ...
       if(hobbyList[i].checked == true){ // 체크가 되어 있을 경우
        ...
        content += hobbyList[i].value + " "; // 하비리스트를 컨텐트에 누적하겠다
...
function cssTest(){
    ...
    container.style.border = "10px soild orange";

    // documnet.querySelector("css선택자")
    ...
    const colors = ['yellow', 'pink']; //배열 선언 및 초기화(처음값을대입했다)

    for(let i = 0; i < divs.length; i++){
        divs[i].style.height = "50%";
        divs[i].style.display = "flex";
        divs[i].style.justifyContent = "center"; // 중심축 방향 가운데
        divs[i].style.alignItem = "center"; // 교차축 방향 가운데
