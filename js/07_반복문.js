...
function check1(){
    ...
    // [조건식] num <= 5 
    //   -> num 변수에 저장된 값이 5 이하면 true -> 반복 0
...
function check11(){
    ...
    // - 곱셈 결과가 1~9까지 1씩 즈가한 수에 2를 곱한 값
...
function check26(){
    ...
    console.log(`1~100사이 ${input}의 배수는 ${count}개 있습니다`)
...
function check28(){
   ...
   while(test <=5 ){
    ...
    let val; // prompt에 입력된 값을 number 타입으로 변환 후 저장할 변수
...
function check29(){
    ...
    while(input !== null){
        ...
        input = prompt("메뉴 변호 입력(1.김밥, 2.라면 3.돈까스")
    ...
    if(rCount > 0){ // 김밥을 주문한 경우
        outputHtml += `<li>라면(${rCount}개) : ${ramen * rCount}원 </li>`;
    }
    if(dCount > 0){ // 김밥을 주문한 경우
    ...
    // #result29의 내용으로 outputHtml 출려(HTML 요소 해석)
