...
function check2(){
    ...
    // 내림 처림(소수점 삭제) : Math.floor(실수);

    // Math.floor(Math.random() *100 +1) -> 1 <= 난수 < 101

    // 1~100 사이 난수를 발생시켜 randomNumber 상수에 저장
...
function ageCheck2(){
    ...
    } else if(age <= 13){ // (age >= 0 && age <= 13)와 같음
        result = "어린이";
    ...
    alert(result);
     ...
     /*
        ...
        html 요소에 작성된 내용 얻어오기 : html요소.innerHTML
    ...
    // 매개 변수(Parameter)
    // - 함수가 호출될 때 전달된 값(전달인자, Argumnet)을
    //  저장하기 위한 변수
    ...
    function calc(op){
        ...
        const number1 = document.getElemnetById("number1");
        const number2 = document.getElemnetById("number2");
