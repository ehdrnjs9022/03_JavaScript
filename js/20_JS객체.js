...
/* 
    ...
    <button onlclick="testFn(this)" > 테스트</button>
...
/* 학생 추가 버튼 클릭 시 입력 된 값을 모두 얻어와 
 ...
 addStudent.addEventListener("click", () =>{
    ...
    // 입력되지 않은 값이 있는지 확인 
    for(let val of valuse){           //valuse값중 하나씩 꺼내봄=val
        if(val.trim() === ''){        //trim 양쪽 공백문자 제거
            alert("모든 값을 입력 해주세요");
            return;
        }
    }
    ...
    const std = new Student(
        Number(valuse[0]),Number(valuse[1]),
        Number(valuse[2]),valuse[3])
...
/*  
    배열.forEach( (item, index) => {  
        // item(변수이름정해서마음대로) : for문 반복 시 0번 인덱스 부터 차례대로 꺼낸 요소
...
const menu = '{"name" : "김밥","price": "4000"}';
    console.log("munu :",menu,typeof menu); //string
