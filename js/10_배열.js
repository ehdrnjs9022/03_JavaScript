...
function check1(){
    ...
    const arr2 = new Array(3); // 3칸 배열(요소 모두 비어 있음)
    const arr3 = []; // 0칸 배열
    const arr4 = ['a','b','c']; //3칸 배열 + 초기화
...
function check2(){
    ...
    // weeks.length === 7
    // weeks의 시작 인덱스 번호 : 0
    // wekks의 마지막 인덱스 번호 : 6 === weeks.length-1
    // index는 0이상의 정수
    // index는 1씩 증가
...
function check4(){
    ...
    console.log("평균균 : " , sum / arr.length);

    // 4) 짝수(0,2,4) 번째 index 요소의 합계 출력
    // 5) 홀수(1,3)   번째 index 요소의 합계 출력

    let oddSum = 0 ; // 홀수 합계 저장할 변수
    let evenSum = 0; // 짝수 합게 저장할 변수
    for(let i = 0; i<arr.length; i++){
...
function check6(){
    ...
    console.log("dArr2[0].length :", dArr2.length);
   
    // 1차원 배열 내에 저장된 배열 요소의 개수
    console.log("dArr2[1] :", dArr2[1]);
    console.log("dArr2[1].length :", dArr2.length);
    
   // 2차원 배열 내에 저장된 1차원 배열 요소의 개수
    console.log("dArr2[2].length :", dArr2.length); //3


    // darr2의 0행 2열에 저장된 값 얻어오기
    console.log("dArr[0][2]: " , dArr2[0][2]); // 30
   
    // darr2의 1행 1열에 저장된 값 얻어오기
    console.log("dArr[1][1]: " , dArr2[1][1]); // 30
   
    // darr2의 2행 0열에 저장된 값 얻어오기
    console.log("dArr[2][0]: " , dArr2[2][0]); // 30

    /* 2차원 배열 인덱스를 이용해서 값 대입 */
    
    // 0행 0열, 1행 0열, 2행 0열 값을 * 10 해서 대입
    dArr2[0][0]=dArr2[0][0] * 10;
    dArr2[1][0] *=10; // 복합 대입 연산자 //기존값에서 10곱해라
    dArr2[2][0] *=10;
...
/* 4행 4열 짜리 2차원 배열에 1~16 사이 난수 배치(중복 X) 
    1) 배열을 콘솔에 출력
    2) prompr를 이용해서 1~16 사이 숫자를 입력 받아
...
function check7(){
    ...
    for(let col=0 ; col<4 ; col++){ // 열 반복
       ...
       // arr[row][col]에 대입
        ...
        index++; // index 를 1증가 시켜 randomArr 다음 요소 지정하기
